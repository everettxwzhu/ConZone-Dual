...
{
	return ((val & (uint32_t)0x000000ffUL) << 24) | ((val & (uint32_t)0x0000ff00UL) << 8) |
		   ((val & (uint32_t)0x00ff0000UL) >> 8) | ((val & (uint32_t)0xff000000UL) >> 24);
}
...
#define BUF_DISP_utf(buf, data, len, ptr, member) snprintf(buf, len, #member)
...
#define set_sb(member, val)                                                                        \
	do {                                                                                           \
		...
		switch (sizeof(t)) {                                                                       \
		case 8:                                                                                    \
			set_sb_le64(member, val);                                                              \
			break;                                                                                 \
		case 4:                                                                                    \
			set_sb_le32(member, val);                                                              \
			break;                                                                                 \
		case 2:                                                                                    \
			set_sb_le16(member, val);                                                              \
			break;                                                                                 \
		}                                                                                          \
	} while (0)
...
#define get_sb(member)                                                                             \
	({                                                                                             \
		...
		switch (sizeof(t)) {                                                                       \
		case 8:                                                                                    \
			t = get_sb_le64(member);                                                               \
			break;                                                                                 \
		case 4:                                                                                    \
			t = get_sb_le32(member);                                                               \
			break;                                                                                 \
		case 2:                                                                                    \
			t = get_sb_le16(member);                                                               \
			break;                                                                                 \
		}                                                                                          \
		t;                                                                                         \
	})
#define get_newsb(member)                                                                          \
	({                                                                                             \
		...
		switch (sizeof(t)) {                                                                       \
		case 8:                                                                                    \
			t = get_newsb_le64(member);                                                            \
			break;                                                                                 \
		case 4:                                                                                    \
			t = get_newsb_le32(member);                                                            \
			break;                                                                                 \
		case 2:                                                                                    \
			t = get_newsb_le16(member);                                                            \
			break;                                                                                 \
		}                                                                                          \
		t;                                                                                         \
	})
...
#define set_cp(member, val)                                                                        \
	do {                                                                                           \
		...
		switch (sizeof(t)) {                                                                       \
		case 8:                                                                                    \
			set_cp_le64(member, val);                                                              \
			break;                                                                                 \
		case 4:                                                                                    \
			set_cp_le32(member, val);                                                              \
			break;                                                                                 \
		case 2:                                                                                    \
			set_cp_le16(member, val);                                                              \
			break;                                                                                 \
		}                                                                                          \
	} while (0)
...
#define get_cp(member)                                                                             \
	({                                                                                             \
		...
		switch (sizeof(t)) {                                                                       \
		case 8:                                                                                    \
			t = get_cp_le64(member);                                                               \
			break;                                                                                 \
		case 4:                                                                                    \
			t = get_cp_le32(member);                                                               \
			break;                                                                                 \
		case 2:                                                                                    \
			t = get_cp_le16(member);                                                               \
			break;                                                                                 \
		}                                                                                          \
		t;                                                                                         \
	})
...
#define F2FS_EXTRA_ISIZE_OFFSET offsetof(struct f2fs_inode, i_extra_isize)
#define F2FS_TOTAL_EXTRA_ATTR_SIZE                                                                 \
	(offsetof(struct f2fs_inode, i_extra_end) - F2FS_EXTRA_ISIZE_OFFSET)

#define F2FS_DEF_PROJID 0 /* default project ID */

#define MAX_INLINE_DATA(node)                                                                      \
	(sizeof(__le32) * (DEF_ADDRS_PER_INODE - get_inline_xattr_addrs(&node->i) -                    \
					   get_extra_isize(node) - DEF_INLINE_RESERVED_SIZE))
#define DEF_MAX_INLINE_DATA                                                                        \
	(sizeof(__le32) * (DEF_ADDRS_PER_INODE - DEFAULT_INLINE_XATTR_ADDRS -                          \
					   F2FS_TOTAL_EXTRA_ATTR_SIZE - DEF_INLINE_RESERVED_SIZE))
#define INLINE_DATA_OFFSET                                                                         \
	(PAGE_CACHE_SIZE - sizeof(struct node_footer) -                                                \
	 sizeof(__le32) * (DEF_ADDRS_PER_INODE + 5 - DEF_INLINE_RESERVED_SIZE))
...
#define MAX_SIT_BITMAP_SIZE                                                                        \
	(SEG_ALIGN(SIZE_ALIGN(F2FS_MAX_SEGMENT, SIT_ENTRY_PER_BLOCK)) * c.blks_per_seg / 8)
...
#define SIZE_OF_RESERVED                                                                           \
	(PAGE_SIZE - ((SIZE_OF_DIR_ENTRY + F2FS_SLOT_LEN) * NR_DENTRY_IN_BLOCK + SIZE_OF_DENTRY_BITMAP))
...
/* for inline dir */
#define NR_INLINE_DENTRY(node)                                                                     \
	(MAX_INLINE_DATA(node) * BITS_PER_BYTE /                                                       \
	 ((SIZE_OF_DIR_ENTRY + F2FS_SLOT_LEN) * BITS_PER_BYTE + 1))
#define INLINE_DENTRY_BITMAP_SIZE(node)                                                            \
	((NR_INLINE_DENTRY(node) + BITS_PER_BYTE - 1) / BITS_PER_BYTE)
#define INLINE_RESERVED_SIZE(node)                                                                 \
	(MAX_INLINE_DATA(node) - ((SIZE_OF_DIR_ENTRY + F2FS_SLOT_LEN) * NR_INLINE_DENTRY(node) +       \
...
static inline const char *blk_zone_type_str(struct blk_zone *blkz)
{
...
static inline const char *blk_zone_cond_str(struct blk_zone *blkz)
{
...
#define ZONE_ALIGN(blks) SIZE_ALIGN(blks, c.blks_per_seg *c.segs_per_zone)
...
static inline double get_best_overprovision(struct f2fs_super_block *sb)
{
	...
	for (; candidate <= end; candidate += diff) {
		reserved =
			(2 * (100 / candidate + 1) + 6) * round_up(usable_main_segs, get_sb(section_count));
...
static inline __le64 get_cp_crc(struct f2fs_checkpoint *cp)
{
	...
	u_int32_t crc = le32_to_cpu(*(__le32 *)((unsigned char *)cp + crc_offset));
...
static inline int parse_root_owner(char *ids, u_int32_t *root_uid, u_int32_t *root_gid)
{
	char *uid = ids;
	char *gid = NULL;
	int i;
...
struct f2fs_nls_ops {
	int (*casefold)(const struct f2fs_nls_table *charset, const unsigned char *str, size_t len,
